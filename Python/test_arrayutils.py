...
class TestFindFunctions(unittest.TestCase):
    ...
    def test_find_if(self):
        a = [27, 82, 41, 124, 62, 31, 94, 47]
        pred = lambda n : n > 100
        self.assertEqual(arrayutils.find_if(a, pred), 3)
        self.assertEqual(arrayutils.find_if(a, pred, lo=2, hi=6), 3)
        self.assertEqual(arrayutils.find_if(a, pred, lo=0, hi=3), -1)
        self.assertEqual(arrayutils.find_if(a, pred, lo=4, hi=-1), -1)
